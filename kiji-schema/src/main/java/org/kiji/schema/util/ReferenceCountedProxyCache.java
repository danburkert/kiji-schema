...
public final class ReferenceCountedProxyCache<K, V extends Closeable> implements Closeable {
   ...
   * @param trackResources Whether the cached values should be registered with the debug resource
   *     tracker.
   * @param trackProxies Whether the created proxy values should be registered with the debug
   *     resource tracker.
   */
  ...
  public static <K, V extends Closeable> ReferenceCountedProxyCache<K, V> create(
    ...
    return new ReferenceCountedProxyCache<K, V>(iface, cacheLoader, false, false);
  }
  ...
  createWithResourceTracking(
    ...
    return new ReferenceCountedProxyCache<K, V>(iface, cacheLoader, true, false);
  }

  /**
   * Create a {@code ReferenceCountedCache} using the supplied function to create cached values
   * on demand with debug resource tracking of proxies.
   *
   * <p>
   *   The function may be evaluated with the same key multiple times in the case that the key
   *   becomes invalidated due to the reference count falling to 0. A function need not handle the
   ...
   *
   * <p>
   *   The returned proxy objects will be registered with the {@link DebugResourceTracker}, and
  ...
  public static <K, V extends Closeable> ReferenceCountedProxyCache<K, V> createWithProxyTracking(
    ...
    return new ReferenceCountedProxyCache<K, V>(iface, cacheLoader, false, true);
  }
  ...
  public V get(K key) {
    ...
    final ReferenceCountedProxy<K, V> proxy =
        new ReferenceCountedProxy<K, V>(value, key, mCache, mTrackProxies);
    if (mTrackProxies) {
      DebugResourceTracker.get().registerResource(proxy);
    }
   ...
   *
   * <p>
   *   The proxy may optionally unregister itself with the debug resource tracker when closed.
   * </p>
   *
  ...
  private static final class ReferenceCountedProxy<K, V extends Closeable>
     ...
     * @param trackedProxy Whether this proxy should unregister itself from the debug resource
     *     tracker on close.
     */
