 ...
 *
 * <p>
 *   The {@code KijiResult} API is not compile-time type safe if {@link #narrowView} is used
 *   inappropriately. {@code narrowView} allows the caller to specify the value-type of
 ...
 *
 * <p>
 *   {@code KijiResult} instances must be closed if their data request includes paged column after.
 *   {@code KijiResult}s returned from {@link #narrowView} must be closed independently of the
 *   {@code KijiResult} they are created from, if the narrowed view contains paged columns.
 * </p>
 *
 ...
 *    inferred. Users may request a more specific type when calling {@link #narrowView},
...
public interface KijiResult<T> extends Iterable<KijiCell<T>>, Closeable {
   ...
   *   (or wrong), a runtime {@link java.lang.ClassCastException} will be thrown when the returned
   *   {@code KijiResult} is used. See the Type Safety section of {@code KijiResult}'s
   ...
   * @param <U> type of the value in the given column.
   * @return a {@code KijiResult} which contains only the provided column.
   */
  <U> KijiResult<U> narrowView(KijiColumnName column);
  ...
  public static final class Helpers {
     ...
     * </p>
     *
     * @param result to get first cell from.
     ...
     *   and the version count is not important, set the max number of versions to retrieve in the
     *   {@link KijiDataRequest} for each column to {@code 1} to avoid retrieving unnecessary
     *   {@code KeyValues}s.
     * </p>
     *
     * @param result the {@code KijiResult} from which to retrieve versions.
     ...
     * <p>
     *   If the provided {@code KijiResult} will only be used retrieving the columns of the row,
     *   the {@link org.kiji.schema.filter.StripValueRowFilter} or
