...
public final class ResultDecoders {
  ...
  public static <T> Function<KeyValue, KijiCell<T>> getDecoderFunction(
    ...
    if (column.isFullyQualified()) {
      final KijiCellDecoder<T> decoder =
          decoderProvider.getDecoder(column.getFamily(), column.getQualifier());
    ...
    if (family.isMapType()) {
      return new MapFamilyDecoder<T>(
          translator,
          decoderProvider.<T>getDecoder(column.getFamily(), column.getQualifier()));
  ...
  /**
   * A function which will decode {@link KeyValue}s from a group-type column.
   *
   * <p>
    ...
    public KijiCell<T> apply(final KeyValue keyValue) {
      if (!Arrays.equals(mLastQualifier, keyValue.getQualifier())) {
        try {
          ...
          mLastDecoder =
              mDecoderProvider.getDecoder(mLastColumn.getFamily(), mLastColumn.getQualifier());
        } catch (NoSuchColumnException e) {
          mLastDecoder = null;
          mLastColumn = null;
          mLastQualifier = null;
