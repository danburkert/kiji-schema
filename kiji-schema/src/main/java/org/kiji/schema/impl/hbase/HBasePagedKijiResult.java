...
public class HBasePagedKijiResult<T> implements KijiResult<T> {
  ...
  private final AtomicBoolean mDebugRegistered = new AtomicBoolean(false);
  ...
  public Iterator<KijiCell<T>> iterator() {
    if (mDebugRegistered.compareAndSet(false, true)) {
  ...
  public <U extends T> HBasePagedKijiResult<U> narrowView(final KijiColumnName column) {
    ...
    if (narrowRequest.equals(mDataRequest)) {
      return (HBasePagedKijiResult<U>) this;
    }
  ...
  public void close() throws IOException {
    try {
      ...
      if (mDebugRegistered.compareAndSet(true, false)) {
