...
public final class MaterializedKijiResult<T> implements KijiResult<T> {
  ...
  public <U extends T> KijiResult<U> narrowView(final KijiColumnName column) {
    ...
    for (Column columnRequest : narrowRequest.getColumns()) {
      ...
      if (exactColumn != null) {
        ...
        final List<KijiCell<T>> familyResults =
            getQualifiedColumnCells(requestedColumn, familyColumn);
        narrowedColumns.put(requestedColumn, (List<KijiCell<U>>) (Object) familyResults);
      }
    }
   ...
   */
  private static <T> List<KijiCell<T>> getQualifiedColumnCells(
