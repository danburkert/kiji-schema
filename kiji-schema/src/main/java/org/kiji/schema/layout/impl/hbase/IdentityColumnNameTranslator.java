 ...
 * {@inheritDoc}.
 *
 * <p> The identity mapping uses the Kiji locality group as the column family and the Kiji
 * family:qualifier as the qualifier.</p>
 *
...
public final class IdentityColumnNameTranslator extends KijiColumnNameTranslator {
  ...
  public KijiColumnName toKijiColumnName(TranslatedColumnName translatedColumnName)
    ...
    LOG.debug("Translating HBase column name {} to Kiji column name.", translatedColumnName);

    final String localityGroupName = Bytes.toString(translatedColumnName.getFamily());
    ...
    final byte[] hbaseQualifier = translatedColumnName.getQualifier();
    final int index = ArrayUtils.indexOf(hbaseQualifier, SEPARATOR);
    if (index == -1) {
      throw new NoSuchColumnException(String.format(
          "Missing separator in HBase column %s.", translatedColumnName));
    }
  ...
  public TranslatedColumnName toTranslatedColumnName(KijiColumnName kijiColumnName)
    ...
    return new TranslatedColumnName(localityGroupBytes, hbaseQualifierBytes);
  }
