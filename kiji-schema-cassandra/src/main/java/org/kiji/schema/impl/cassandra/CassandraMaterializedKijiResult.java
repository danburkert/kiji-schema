...
/**
 * A {@link KijiResult} backed by an Cassandra {@link Result}.
 *
 * @param <T> The type of {@code KijiCell} values in the view.
 */
@ApiAudience.Private
public final class CassandraMaterializedKijiResult<T> implements KijiResult<T> {
  ...
  /**
   * Create a {@code KijiResult} backed by an Cassandra {@link Result}.
   *
   ...
   * @param columnResults The materialized Cassandra results.
   */
   ...
   * @param entityId The entity ID of the row to which the {@code Result} belongs.
   * @param dataRequest The data request which defines the columns in this {@code KijiResult}.
   * @param result The backing Cassandra result.
   * @param layout The Kiji table layout of the table.
   * @param columnTranslator The Kiji column name translator of the table.
   * @param decoderProvider The Kiji cell decoder provider of the table.
   * @param <T> The type of {@code KijiCell} values in the view.
   * @return A {@code KijiResult} backed by an Cassandra {@code Result}.
   */
  public static <T> CassandraMaterializedKijiResult<T> create(
      ...
      final CassandraKijiTable layout,
      final CassandraColumnNameTranslator columnTranslator,
      final CellDecoderProvider decoderProvider
  ) {
    Preconditions.checkArgument(dataRequest.isPagingEnabled());
  ...
  public <U extends T> CassandraMaterializedKijiResult<U> narrowView(final KijiColumnName column) {
    ...
    if (narrowRequest.equals(mDataRequest)) {
      return (CassandraMaterializedKijiResult<U>) this;
    }
    ...
    return new CassandraMaterializedKijiResult<U>(
        mEntityId,
        mDataRequest,
        mLayout,
        mColumnTranslator,
  ...
  private static List<KeyValue> getQualifiedColumnKeyValues(
    ...
    try {
      final CassandraColumnName cassandraColumn =
          translator.toCassandraColumnName(columnRequest.getColumnName());
      family = cassandraColumn.getFamily();
      qualifier = cassandraColumn.getQualifier();
    } catch (NoSuchColumnException e) {
      throw new IllegalArgumentException(e);
    }

    final KeyValue start = new KeyValue(rowkey, family, qualifier, Long.MAX_VALUE, EMPTY_BYTES);
    // Cassandra will never return a KeyValue with a negative timestamp, so -1 is fine for exclusive end
  ...
  private static KijiColumnName getKeyValueColumnName(
    ...
    final CassandraColumnName hBaseColumnName = new CassandraColumnName(kv.getFamily(), kv.getQualifier());
    try {
      return translator.toKijiColumnName(hBaseColumnName);
    } catch (NoSuchColumnException nsce) {
      // This should not happen since it's only called on data returned by Cassandra.
