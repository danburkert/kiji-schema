...
/**
 * A {@link KijiResult} backed by Cassandra.
 *
 * @param <T> The type of {@code KijiCell} values in the view.
 */
@ApiAudience.Private
public final class CassandraKijiResult<T> implements KijiResult<T> {

  private final KijiDataRequest mDataRequest;
  private final CassandraMaterializedKijiResult<T> mMaterializedResult;
  private final CassandraPagedKijiResult<T> mPagedResult;

  /**
   * Initialize a new {@link CassandraKijiResult}.
   *
  ...
  private CassandraKijiResult(
      ...
      final CassandraMaterializedKijiResult<T> materializedResult,
      final CassandraPagedKijiResult<T> pagedResult
  ) {
  ...
  /**
   * Create a new {@link CassandraKijiResult}.
   *
   ...
   * @return an {@code CassandraKijiResult}.
  ...
  public static <T> KijiResult<T> create(
    ...
    if (unpagedRequest.isEmpty()) {
      ...
      return new CassandraKijiResult<T>(dataRequest, materializedKijiResult, pagedKijiResult);
    }
  }
  ...
  public <U extends T> KijiResult<U> narrowView(final KijiColumnName column) {
    ...
    for (Column columnRequest : narrowRequest.getColumns()) {
      ...
      if (containsPagedColumns && containsUnpagedColumns) {
        return new CassandraKijiResult<U>(
            narrowRequest,
